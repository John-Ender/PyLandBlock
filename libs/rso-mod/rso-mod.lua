...
local rso_settings = rsoSettings()

-- cached setting value
local region_size = rso_settings["rso-region-size"].value
...
if rso_settings["rso-use-donuts"].value then
...
local function modify_resource_size(resourceName, resourceSize, startingArea)
	...
	if not startingArea then
		resourceSize = math.ceil(resourceSize * rso_settings["rso-global-size-mult"].value)
	end
...
--		log("Index rolled "..index.." shapes amount "..#meta_shapes)
	...
	if validCount > 0 then
		local removeTrees = rso_settings["rso-remove-trees"].value
		local revealResources = rso_settings["rso-reveal-spawned-resources"].value
		local revealChunks = {}
...
--		end
		...
		local richnessMultiplier = rso_settings["rso-global-richness-mult"].value
		
		if startingArea then
			richnessMultiplier = rso_settings["rso-starting-richness-mult"].value
		end
...
--		end
		...
		if startingArea and not rso_settings["rso-infinite-ores-in-start-area"].value then
			infiniteOrePresent = false
		end

		local infiniteResourceSpawnThreshold = rso_settings["rso-infinite-ore-threshold"].value
...
local function spawn_entity(surface, ent, r_config, x, y, rng)
	if not rso_settings["rso-biter-generation"].value then return end
	...
	size = size * rso_settings["rso-enemy-base-size"].value
		...
		if resConfig.starting and resConfig.valid and resConfig.allotment > 0 then 
			...
			if resConfig.starting.probability > 0 and prob <= resConfig.starting.probability then
				...
				while (radius < maxRadius) and (total < min_threshold) do
					local angle = rng() * pi * 2
					local dist = radius + rng(radius / 2)
...
local function build_config_data(surface)
	...
	for res_name, resConf in pairs(config) do
		if resConf.valid then -- only add valid resources
			...
			if not rso_settings["rso-oil-in-start-area"].value and resConf.type == "resource-liquid" then
				resConf.starting = nil
			end
			
			if not rso_settings["rso-ore-in-start-area"].value and resConf.type == "resource-ore" then
				resConf.starting = nil
			end
			...
			if addResource then
				...
				if res_conf.multi_resource and rso_settings["rso-multi-resource-active"].value then
					local new_list = {}
...
local function roll_region(surface, c_x, c_y)
	...
	if regions[r_x] and regions[r_x][r_y] then
		...
		local resourceSetting = rso_settings["rso-resource-chance"].value
...
--	log(serpent.dump(r_data))
	...
	if r_data[c_x] and r_data[c_x][c_y] then
		...
		local richness_distance_factor = rso_settings["rso-distance-exponent"].value
		local fluid_richness_distance_factor = rso_settings["rso-fluid-distance-exponent"].value
		local size_distance_factor = rso_settings["rso-size-exponent"].value
...
function localGenerateChunk( event )
...
script.on_event(defines.events.on_runtime_mod_setting_changed, function(event)
	if event.setting == "rso-region-size" then
		...
		region_size = rso_settings["rso-region-size"].value
		REGION_TILE_SIZE = CHUNK_SIZE*region_size
	elseif event.setting == "rso-enemy-chance" then
		for surfaceIndex, surfaceData in pairs(global.surfaces) do
			for index,v in pairs(surfaceData.configIndexed) do
				if v.absolute_probability then
					v.absolute_probability = rso_settings["rso-enemy-chance"].value
				end
			end
		end
